...
def num_of_model():
    ...
    found = False
    while (not found):
        filepath = '../skin_temperature_prediction_models/prediction_model_'+str(i)
        try:
...
class SkinTemperaturePredictionModel:
    ...
    def train(self):
        ...
        if (decision == "y" or decision == "Y"):
            save_path = '../skin_temperature_prediction_models/prediction_model_'+str(num_of_model()+1)+'.h5'
    ...
    def predict(self, inputs, delta_time, time):
        ...
        for data in inputs:
            package_dict = {}
            for key in ['temp','hum','outdoor_temp','outdoor_hum','body','set_temp','set_fanspeed']:
        ...
        return prediction.item() + package[1]['body']
...
def create_model():
    return SkinTemperaturePredictionModel(**parameters)
